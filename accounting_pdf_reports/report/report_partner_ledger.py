...
class ReportPartnerLedger(models.AbstractModel):
    ...
    def _lines(self, data, partner,initial_balance=0):
        full_account = []
        ...
        # 👇 Get initial balance
        initial_balance = self._get_initial_balance_for_partner(data, partner.id)

        # 👇 Add synthetic line for Initial Balance
        full_account.append({
            'date': '',
            'code': '',
            'a_code': '',
            ...
            'move_name': '',
            'name': '',
            'displayed_name': _('Initial Balance'),
        ...
        for r in res:
            ...
            sum_balance += r['debit'] - r['credit']
            r['progress'] = sum_balance
    ...
    def _sum_partner(self, data, partner, field):
        ...
        if field == 'debit' and initial_balance > 0:
            result += initial_balance
        elif field == 'credit' and initial_balance < 0:
            result += abs(initial_balance)
    ...
    def _get_report_values(self, docids, data=None):
        ...
        return {
            ...
            'time': time,
            'lines': self._lines,
            'sum_partner': self._sum_partner, # ✅ Add this
        }

    def _get_initial_balance(self, data, partner_ids):
        domain = [('partner_id', 'in', partner_ids)]
        if data['computed'].get('account_ids'):
            domain += [('account_id', 'in', data['computed']['account_ids'])]
        if data['computed'].get('move_state'):
            domain += [('move_id.state', 'in', data['computed']['move_state'])]
        ...
        # Sum debit and credit per partner
        result = {}
        for partner_id in partner_ids:
            lines = aml.filtered(lambda l: l.partner_id.id == partner_id)
            debit = sum(lines.mapped('debit'))
            credit = sum(lines.mapped('credit'))
            result[partner_id] = debit - credit
        return result

    def _get_initial_balance_for_partner(self, data, partner_id):
