...
class Module(models.Model):
    ...
    def upgrade_changed_checksum(self, overwrite_existing_translations=False):
        ...
        # Filter to only include modules authored by "IMC"
        changed_modules = changed_modules.filtered(lambda m: m.author == "IMC")
...
class UpgradeConfirmationWizard(models.TransientModel):
    _name = 'upgrade.confirmation.wizard'
    _description = 'Upgrade Confirmation Wizard'

    def action_confirm(self):
        self.ensure_one()
        self.env['ir.actions.server'].browse(
            self.env.ref('module_auto_update.ir_module_module_upgrade_changed_checksum').id
        ).run()
