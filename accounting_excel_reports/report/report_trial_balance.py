...
class ReportTrialBalanceExcel(models.Model):
    ...
    def generate_xlsx_report(self, workbook, data, obj):
        ...
        sheet.write('C6', "initial_balance", format3)
        sheet.write('D6', "Debit", format3)
        sheet.write('E6', "Credit", format3)
        sheet.write('F6', "Balance", format3)
        row = 6
        col = 0
        for account in result['Accounts']:
            sheet.write(row, col, account['code'], format6)
            sheet.write(row, col+1, account['name'], format6)
            sheet.write(row, col+2, account['initial_balance'], format7)
            sheet.write(row, col+3, account['debit'], format7)
            sheet.write(row, col+4, account['credit'], format7)
            sheet.write(row, col+5, account['balance'], format7)
            row += 1
